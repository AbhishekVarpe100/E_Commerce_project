...
function EditMobile() {
  ...
  const mobileData = () => {
    axios.get(`http://localhost:3000/mobile/${id}`)
        .then(product => {
...
function handleUpdate(e) {
  ...
  axios.post(`http://localhost:3000/updatemobile/${id}`, formData)
      .then(res => {
          if (res.data === 'success') {
              ...
              setTimeout(() => {
                  ...
                  setTimeout(() => {
                    navigate('/admin/available/mobile')
                  }, 1000);
  ...
  return (
    <div>
        <Link to='/admin/available/mobile' className='btn btn-warning m-4 w-25'>&#8592; Back</Link>
      <center>
        {success}
        {failure}
