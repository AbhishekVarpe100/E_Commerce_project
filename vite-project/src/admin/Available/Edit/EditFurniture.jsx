...
function EditFurniture() {
  ...
  const furnitureData = () => {
    axios.get(`http://localhost:3000/furniture/${id}`)
        .then(product => {
...
function handleUpdate(e) {
  ...
  axios.post(`http://localhost:3000/updatefurniture/${id}`, formData)
      .then(res => {
          if (res.data === 'success') {
              ...
              setTimeout(() => {
                  ...
                  setTimeout(() => {
                    navigate('/admin/available/furniture')
                  }, 1000);
  ...
  return (
    <div>
      <Link to='/admin/available/furniture' className='btn btn-warning m-4 w-25'>&#8592; Back</Link>
      <center>
        {success}
        {failure}
