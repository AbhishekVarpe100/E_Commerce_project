...
function AAccessories() {
    ...
    const handleDelete = (name) => {
        axios.delete(`http://localhost:3000/deleteaccessories/${name}`)
            .then((res) => {
                if (res.data == 'success') {
    ...
    return (
        <div className='m-4'>
            ...
            <center>
                <h2 className='heading text-white w-25 p-4 m-2'>Accessories</h2>
                <div>{del}</div>
            </center>
            ...
            <div className='row'>
                ...
                {data.length != 0 ? data.map((item) => (
                    ...
                    <div className='border text-white col-md-3 col-lg-2 m-4 myimagetemplate1 p-4 rounded' key={item.id} >
                        ...
                        <hr />
                        <div className="card-body">
                            <Link className='btn btn-primary w-100' to={`/admin/available/edit/${item.id}`}>Edit</Link>
