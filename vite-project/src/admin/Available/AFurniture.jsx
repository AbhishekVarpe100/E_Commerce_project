...
function AFurniture() {
    ...
    const handleDelete = (name) => {
        axios.delete(`http://localhost:3000/deletefurniture/${name}`)
            .then((res) => {
                if (res.data == 'success') {
    ...
    return (
        <div className='m-4'>
            ...
            <center>
                <h2 className='heading text-white w-25 p-4 m-2'>Furniture / Decor</h2>
                {del}
            </center>
            ...
            <div className='row'>
                {data.length != 0 ? data.map((item) => (
                    ...
                    <div className='border text-white col-md-3 col-lg-2 m-4 myimagetemplate1 p-4 rounded' key={item.id} >
                        ...
                        <hr />
                        <div className="card-body">
                            <Link className='btn btn-primary w-100' to={`/admin/available/editfurniture/${item.id}`}>Edit</Link>
